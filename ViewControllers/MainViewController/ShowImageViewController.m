...
//    [self performSelectorInBackground:@selector(changeNavigationItemTitle) withObject:nil];
}

- (IBAction)saveToPhotosAction:(id)sender {
    NSArray *visibleIndexPaths = [_collectionView indexPathsForVisibleItems];
    NSLog(@"%lu", visibleIndexPaths.count);
    NSIndexPath *indexPathOfVisibleImage = [visibleIndexPaths objectAtIndex:0];
    
    NSURL *url = [[NSURL alloc] initWithString:[_URLsOfImagesToShow objectAtIndex:indexPathOfVisibleImage.row]];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return _URLsOfImagesToShow.count;
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *currentImageLink = [_URLsOfImagesToShow objectAtIndex:indexPath.row];
